# Cerberus AI — one-liner beta bootstrapper
...
Write-Host ""
Write-Host "  CERBERUS AI (BETA BUILD)" -ForegroundColor Magenta
...
try {
    ...
    # Find the latest release containing "beta" in the tag name
    $rel = $releases | Where-Object { $_.tag_name -match "beta" } | Select-Object -First 1
    if (-not $rel) {
        # Fallback to the absolute newest release in the list
        $rel = $releases[0]
    }
    ...
    $installScript    = Join-Path $tmp "cerberus-install-beta.ps1"
    ...
    Write-Host ""
    Write-Host "  Done. Launch Cerberus Beta from the Start Menu." -ForegroundColor Green
